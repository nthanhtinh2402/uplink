...
const requestQueue = []; // Hàng đợi để lưu các yêu cầu
...
async function processQueue() {
    ...
    isProcessing = true;
    const { productUrl, res } = requestQueue.shift(); // Lấy yêu cầu đầu tiên trong hàng đợi
    ...
    try {
        ...
        if (downloadLink) {
            ...
            res.json({ error: 'Không tìm thấy link tải.' });
        }
        console.log('\n━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
        ...
        isProcessing = false;
        processQueue(); // Tiếp tục xử lý yêu cầu tiếp theo trong hàng đợi
    }
}
...
app.get('/getlink', (req, res) => {
    ...
    // Bắt đầu xử lý nếu chưa có yêu cầu nào đang được xử lý
    processQueue();
});

// Khởi động server + trình duyệt
(async () => {
